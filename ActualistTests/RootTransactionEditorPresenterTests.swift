...
extension RootTransactionEditorPresenterTests {
    @Test func initialRulePreviewRunsOnceAcrossRepeatedPreparation() async throws {
        ...
        let bundle = try await support.makeOpenedWritableStoreBundle(additionalFixtureSQL: """
            ...
            INSERT INTO rules VALUES (
                ...
                '[{"field":"imported_payee","op":"contains","value":"Coffee","type":"string"}]',
                '[{"op":"append-notes","value":" reviewed"}]', 'and', 0
            );
            """)
        let state = try support.makeAppState(for: bundle)
        let presenter = RootTransactionEditorPresenter()
        #expect(presenter.present(using: state, prefill: .init(
            accountID: "checking", amountMinorUnits: 1234, payeeName: "Coffee", notes: "Draft"
        )))
        let session = try #require(presenter.presentation)
        await session.prepare(using: state)
        #expect(session.model.notes == "Draft reviewed")
        await session.prepare(using: state)
        #expect(session.model.notes == "Draft reviewed")
        #expect(session.model.amountDigits == "1234")
    }
