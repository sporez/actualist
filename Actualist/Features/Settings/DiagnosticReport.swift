...
enum ActualistDiagnosticReportBuilder {
    static func make(
        ...
        var lines: [String] = [
            ...
            "Cached account transaction feeds: \(store.transactionFeedPagesByKey.keys.filter { if case .account = $0.scope { true } else { false } }.count)",
            "Cached spending feeds: \(store.transactionFeedPagesByKey.keys.filter { $0.scope == .spending }.count)",
